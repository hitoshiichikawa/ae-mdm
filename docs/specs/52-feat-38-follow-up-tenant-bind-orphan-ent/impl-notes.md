...
| 3.1（signup_url_name 永続化） | task 1（migration 0017 で `signup_url_name` 列追加）+ task 2.1（型フィールド追加）/ 永続化往復は task 7.1、Insert 配線は task 3.1/5.1 |

> 上表は task 進行に伴い追記する（本 task 2.1 までが担保する AC を記載）。
